...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col font-round selection:bg-mario-red selection:text-white relative z-10 overflow-hidden">
      ...
      {/* 1. Header Area - User Profile */}
      <header className="pt-4 pb-2 px-4 md:px-6 flex items-center justify-between relative z-20">
        <div className="bg-gradient-to-r from-mario-red to-orange-500 text-white px-4 py-2 md:px-6 md:py-3 rounded-xl font-pixel text-sm md:text-xl tracking-widest shadow-pixel border-4 border-white flex items-center gap-3 transform rotate-[-1deg]">
          <BookOpen fill="white" size={20} className="md:w-7 md:h-7" />
          <span className="hidden md:inline">Parf's Book Adventure V2.0</span>
          <span className="md:hidden">Parf's Adventure</span>
        </div>

        {/* User Info Badge */}
        <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm p-1.5 pr-2 rounded-full border-4 border-black shadow-pixel">
            <div 
      ...
      {/* 2. Main Content Area */}
      <main className="flex-1 flex overflow-hidden p-4 gap-4 md:gap-6 max-w-6xl mx-auto w-full flex-col md:flex-row">
        
        {/* Left: Book List */}
        <section className="flex-1 bg-white/90 rounded-[1.5rem] shadow-pixel-lg p-4 flex flex-col border-4 border-black backdrop-blur-sm overflow-hidden">
